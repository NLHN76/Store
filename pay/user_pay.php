...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_code_session = $user_code;

    if (!$name_post || !$email_post || !$phone_post || !$address_post) {
        echo "Vui lòng điền đủ thông tin.";
    } else {
        $stmt = $conn->prepare("INSERT INTO payment (customer_name, customer_email, customer_phone, customer_address, user_code, product_code, product_name, product_quantity, total_price, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        if ($stmt === false) {
            die("Lỗi Prepare Statement: " . htmlspecialchars($conn->error));
        }

        $stmt->bind_param("sssssssiis", $name_post, $email_post, $phone_post, $address_post, $user_code_session, $productCodesString, $productDetailsString, $itemCount, $totalPrice, $productCategoriesString);

        if ($stmt->execute()) {
            sendConfirmationEmail($name_post, $email_post, $totalPrice, $cart, $address_post, $productCodes, $user_code_session, $productCategories, $productDetailsList);
...
function sendConfirmationEmail($name, $email, $totalPrice, $cart, $address, $productCodes, $user_code, $productCategories, $productDetailsList) {
    ...
    try {
        ...
        $mail->Body = generateEmailBody($name, $totalPrice, $cart, $address, $productCodes, $user_code, $productCategories, $productDetailsList);
...
<div class="container">
    ...
    <?php if (!$isPaymentConfirmed): // Chỉ hiển thị form nếu chưa xác nhận thanh toán ?>
    ...
    <div id="qr-code">
        ...
        <p>Cảm ơn bạn đã đặt hàng! Vui lòng kiểm tra email xác nhận. Khi thanh toán bằng chuyển khoản, hãy ghi rõ Mã Khách Hàng (<?php echo htmlspecialchars($user_code); ?>) trong nội dung chuyển khoản.</p>
    </div>
